...
class ExpertProcess(QThread):
    ...
    def __init__(self, expert_mode, interface_language, translation_language, user_text, user_message, chat_context_status, chat_context, ai_model, tts_voice):
        ...
        if chat_context:
            chat_context = ('The text between <chat_history> and </chat_history> is chat history '
                            'between the user (USER) and you (AI).'
                            + '<chat_history>'
                            + chat_context
                            + '</chat_history>')
        ...
        if expert_mode == 'free_mode':
            if self.expert_role == '':
                self.expert_role = 'You may ask for a specific role at first.'
